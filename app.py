...
def recommend_popular_similar(movie_name: str, n: int = 10, min_ratings: int = 10):
    ...
    df = movies_full.copy()
    df = df[df['title'] != movie_name]  # exclude the same movie
    df['overlap'] = df['genres'].apply(lambda g: jaccard(set(str(g).split('|')), target))

    # candidates = any overlap; fallback to all if none
    cand = df[df['overlap'] > 0]
    if cand.empty:
        cand = df

    # popularity filter (but keep fallback if empty)
    ...
    cols = ['title', 'genres', 'avg_rating', 'num_ratings']
...
def fetch_poster_url_omdb(title: str) -> str | None:
    ...
    try:
        ...
        if _is_valid_url(r.get("Poster")):
            return r.get("Poster")

        # 2) Fallback: search and pick first valid poster (prefer type=movie)
...
def render_cards(df: pd.DataFrame):
    """Render a grid of posters with titles; show placeholder if missing."""
