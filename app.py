# app.py — Simple Movie Recommender (no posters)
...
# ===============================
# Load data (path-safe for Streamlit Cloud)
# ===============================
@st.cache_data
def load_data():
    ...
    stats.rename(columns={"mean": "avg_rating", "count": "num_ratings"}, inplace=True)

    full = movies.merge(stats, on="movieId", how="left")
    full["num_ratings"] = full["num_ratings"].fillna(0)
    full["avg_rating"] = full["avg_rating"].fillna(0)
    return full

movies_full = load_data()

# ===============================
# Simple recommender (genre overlap + popularity)
# ===============================
def recommend_popular_similar(movie_name: str, n: int = 10, min_ratings: int = 10):
    row = movies_full.loc[movies_full["title"] == movie_name]
    if row.empty:
        return None

    target_genres = set(str(row["genres"].values[0]).split("|"))
    ...
    df["overlap"] = df["genres"].apply(lambda g: jaccard(set(str(g).split("|")), target_genres))

    candidates = df[df["overlap"] > 0]
    if candidates.empty:
        candidates = df

    filtered = candidates[candidates["num_ratings"] >= min_ratings]
    if filtered.empty:
        filtered = candidates
